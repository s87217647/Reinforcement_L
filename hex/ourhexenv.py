...
class OurHexGame(AECEnv):
    ...
    def step(self, action):
        ...
        if action == self.board_size * self.board_size:
            if self.agent_selection == "player_1" or not self.is_pie_rule_usable:
                error = True
            ...
            if self.check_winner(marker) and not error:
                print(self.agent_selection + " won " + "(player 1: red player 2:blue)")
